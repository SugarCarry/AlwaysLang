...
# AlwaysLang
可以为不同软件固定不同输入语言，支持 ENG / 中 / 한 自动跟随切换和输入位置语言浮层显示。[去下载](https://github.com/SugarCarry/AlwaysLang/releases)
...
# 功能特性
* 为每个软件单独设置目标输入语言，支持 ENG / 中 / 한，切换到对应软件时自动跟随切换
* 一键 `Always ENG`，在所有软件中始终保持英文输入
* 输入位置语言跟随浮层，在光标附近显示当前输入语言，浮窗不透明度可调
* 支持 CMD / PowerShell 等控制台窗口的语言跟随与浮层显示
...
# 使用说明
1. 把软件或其快捷方式拖入列表，或点击 `Add an APP` 添加
2. 为每个软件选择目标语言（ENG / 中 / 한）并设置是否启用、是否开启大写锁定
3. 点击绿色开始按钮启动跟随，切换到对应软件时会自动切换到设定的输入语言
4. 如需在所有软件中始终保持英文，直接点击 `Always ENG`
5. 目标语言对应的输入法需已在 Windows 语言设置中安装，否则会提示先去安装
